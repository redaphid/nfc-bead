...
That writes `tmp/latest/rezz_multicolor.3mf` with:
...
- **Centauri Carbon 2 printer profile** preserved from the reference 3MF in `tmp/latest/r1_extracted/` — process preset, machine config, filament slots all match what's on your slicer.

Drag `tmp/latest/rezz_multicolor.3mf` into Elegoo Slicer, verify the layer preview shows red body + black spiral at the right Z, and press slice.

## Reference 3MF template (one-time setup)

`nfc-build-rezz-3mf` reads printer/process settings from `tmp/latest/r1_extracted/`. To populate that:
...
2. **Save Project As** → `tmp/latest/r1.3mf`.
3. Extract it: `cd tmp/latest && unzip r1.3mf -d r1_extracted/` (the bash way) or right-click in Explorer → 7-zip → extract to `r1_extracted\`.
4. From now on, every `nfc-build-rezz-3mf` run produces a 3MF that opens with that same printer/process profile.
...
If the press-fit feels loose, increase `PEG_CLEARANCE` in `beads/rezz/build_rezz.py` (currently 0.1 mm); rebuild + reprint. Too tight: decrease.
...
| Path | What |
|---|---|
| `tmp/latest/rezz_multicolor.3mf` | **The one to drag into Elegoo Slicer** |
...
| `tools/build_rezz_3mf.py` | The 3MF builder (`nfc-build-rezz-3mf`) |
...
| `beads/rezz/build_rezz.py` | rezz-specific build pipeline (on `rezz` branch) |
