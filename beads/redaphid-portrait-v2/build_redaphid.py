...
BEAD_DIR      = os.path.join(REPO_DIR, "beads", "redaphid-portrait-v2")
SVG_PATH      = os.path.join(BEAD_DIR, "silhouette.svg")
HAIR_SVG_PATH = os.path.join(BEAD_DIR, "hair.svg")

TARGET_WIDTH  = 25.0           # mm (kandi-bracelet bead size)
# v2 "new design": asymmetric thin halves + single-half string hole (recipe
# gotchas #31/#23). The THICK Bottom hosts the NFC pocket, peg sockets and the
# string hole; the THIN Top carries the hair/eyes decoration + peg bases, so the
# decorated front is slimmer. Pegs stay on Top (this charm's chosen layout).
BOTTOM_THICK  = 2.0            # mm — thick half (sockets + NFC + hole live here)
TOP_THICK     = 1.5            # mm — thin decorated half
THICKNESS     = BOTTOM_THICK + TOP_THICK   # 3.5 mm body (was 4.0 symmetric)
...
HOLE_DIA      = 1.2            # mm — shrunk for the single-half hole (Kandi cord);
                              # in the 2.0 mm Bottom half this leaves ~0.4 mm walls
...
# Peg friction-fit
PEG_DIAMETER  = 2.0
PEG_HEIGHT    = 1.0            # mm — short pegs for the thin halves
PEG_CLEARANCE = 0.05          # mm radial — proven snap-fit (was 0.1, too loose)
PEG_CHAMFER   = 0.3           # mm — lead-in taper on the peg tip (gotcha #30) so
                              # pegs self-start into sockets instead of being forced
...
def drill_string_hole(mesh):
    d = mesh.dimensions
    # Single-half hole: drill it centered in the THICK Bottom half (gotcha
    # #23/#31), not on the split plane. Compute z live (the mesh is centered),
    # so the tube sits fully inside Bottom with ~0.4 mm walls.
    z_min = min(v.co.z for v in mesh.data.vertices)
    z_hole = z_min + BOTTOM_THICK / 2.0
    ...
    clean_mesh(mesh)
    mesh.name = "FullBead"
    print(f"[redaphid] string hole d={HOLE_DIA} drilled @ y={HOLE_Y} z={z_hole:.2f} (in Bottom half)")


def split_halves(mesh):
    z_min = min(v.co.z for v in mesh.data.vertices)
    z_max = max(v.co.z for v in mesh.data.vertices)
    z_mid = z_min + BOTTOM_THICK          # ASYMMETRIC seam: thick Bottom, thin Top
    box_size = 200
...
def add_pegs(half, pegs, mode):
    ...
    """
    peg_r = PEG_DIAMETER / 2.0
    chamf = PEG_CHAMFER if PEG_CHAMFER > 0 else 0.0
    shaft_h = PEG_HEIGHT - chamf
    tip_r = max(peg_r - chamf, 0.2)
    if mode == 'from_bottom_up':
        z_face = max((half.matrix_world @ v.co).z for v in half.data.vertices)
        d = +1.0                                  # pegs grow upward, tip up
    elif mode == 'from_top_down':
        z_face = min((half.matrix_world @ v.co).z for v in half.data.vertices)
        d = -1.0                                  # pegs grow downward, tip down
    else:
        raise ValueError(f"unknown peg mode: {mode}")
    for i, (px, py) in enumerate(pegs):
        # full-diameter shaft (grips)
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=32, radius=peg_r, depth=shaft_h,
            location=(px, py, z_face + d * shaft_h / 2.0))
        boolean_op(half, bpy.context.active_object, 'UNION', f"Peg{i}")
        if chamf > 0:
            # chamfered tip: cone frustum, full radius at the shaft end ->
            # tip_r at the far end so the peg self-starts into its socket.
            # OVERLAP the shaft by 0.15mm (shift the cone back toward the shaft)
            # so the UNION actually merges — a coplanar butt-join leaves the tip
            # as a separate body.
            OVERLAP = 0.15
            bpy.ops.mesh.primitive_cone_add(
                vertices=32,
                radius1=(peg_r if d > 0 else tip_r),   # cone bottom (z-) radius
                radius2=(tip_r if d > 0 else peg_r),   # cone top (z+) radius
                depth=chamf,
                location=(px, py, z_face + d * (shaft_h + chamf / 2.0 - OVERLAP)))
            boolean_op(half, bpy.context.active_object, 'UNION', f"PegTip{i}")
    clean_mesh(half)
...
def export_stls(bottom, top, hair, deco):
    """Direct headless export of print-oriented STLs (no MCP export skill).
    Bottom: built back-face-down already -> just drop to z=0. Top assembly
    (Top+Hair+Decoration): pegs hang DOWN and the show face is up, so flip the
    whole assembly 180° about world X (show face -> plate, pegs -> up) keeping
    the three aligned, then drop to z=0."""
    import math as _m
    print_dir = os.path.join(BEAD_DIR, "print")
    os.makedirs(print_dir, exist_ok=True)

    # flip the top assembly about world origin (CURSOR pivot keeps them aligned)
    sc = bpy.context.scene
    sc.cursor.location = (0.0, 0.0, 0.0)
    prev_pivot = sc.tool_settings.transform_pivot_point
    sc.tool_settings.transform_pivot_point = 'CURSOR'
    bpy.ops.object.select_all(action='DESELECT')
    for o in (top, hair, deco):
        o.select_set(True)
    bpy.context.view_layer.objects.active = top
    bpy.ops.transform.rotate(value=_m.pi, orient_axis='X')
    bpy.ops.object.transform_apply(rotation=True)
    sc.tool_settings.transform_pivot_point = prev_pivot

    def world_zmin(objs):
        return min((o.matrix_world @ v.co).z for o in objs for v in o.data.vertices)

    # drop Bottom and the Top-assembly to z=0 independently
    bottom.location.z -= world_zmin([bottom])
    dz = world_zmin([top, hair, deco])
    for o in (top, hair, deco):
        o.location.z -= dz
    bpy.context.view_layer.update()

    for obj, name in ((bottom, "Bottom"), (top, "Top"), (hair, "Hair"), (deco, "Decoration")):
        bpy.ops.object.select_all(action='DESELECT')
        obj.select_set(True)
        bpy.context.view_layer.objects.active = obj
        path = os.path.join(print_dir, f"{name}.stl")
        bpy.ops.wm.stl_export(filepath=path, export_selected_objects=True, apply_modifiers=True)
        print(f"[redaphid] exported {path}")
...
def main():
    ...
    # 8.5 Export print-oriented STLs (before the inspection X-offset)
    export_stls(bottom, top, hair, deco)
