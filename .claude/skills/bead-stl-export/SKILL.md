...
| `tools/build_3mf.py` (`nfc-build-3mf`) | host (uv) | **Recommended when you have a slicer template.** Builds a slicer-ready Bambu Studio / Elegoo Slicer 3MF. Pre-merges Top + Decoration, assigns extruders (1 = decoration, 2 = body), disables brim/raft (the actual fix for the "raft above plate" appearance), and inherits the printer profile from a reference 3MF in `tmp/latest/slicer_template/`. Output: `tmp/latest/bead_multicolor.3mf`. |
| `tools/make_3mf.py` (`nfc-make-3mf`) | host (uv) | Generic fallback. Plain 3MF Consortium bundle via `lib3mf` (no Bambu/Elegoo extensions). Use only when you don't have a reference 3MF template. Output: `tmp/latest/bead.3mf`. |
...
# 3. (host shell) build a slicer-ready 3MF
uv run nfc-build-3mf

# 4. drag tmp/latest/bead_multicolor.3mf into Elegoo Slicer
```

The reference 3MF template (`tmp/latest/slicer_template/`) is created once: save a project from your slicer with the printer + filament setup you want preserved, drop the `.3mf` into `tmp/latest/`, and extract it to `slicer_template/`. From then on every `nfc-build-3mf` run produces a 3MF that opens with that profile.
